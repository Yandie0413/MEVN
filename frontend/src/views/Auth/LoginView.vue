...
<template>
  <div class="min-h-screen flex font-sans bg-edu-bg dark:bg-gray-900">
    ...
    <div class="hidden lg:flex flex-col flex-1 bg-edu-navy dark:bg-gray-900 text-white p-12 justify-between relative overflow-hidden">
      <div class="absolute top-0 right-0 w-80 h-80 bg-edu-lime/10 rounded-full -translate-y-1/2 translate-x-1/2"></div>
      ...
      <div class="relative z-10">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="feat in features" :key="feat.label" class="flex items-center gap-3 text-sm">
            ...
            <span class="text-gray-300 dark:text-gray-400">{{ feat.label }}</span>
          </div>
        </div>
      </div>

      <!-- Testimonial -->
      <div class="bg-white/5 dark:bg-gray-800/30 rounded-2xl p-5 relative z-10 border border-white/10 dark:border-gray-700">
        <div class="flex items-center gap-3 mb-3">
          <img src="https://i.pravatar.cc/40?img=12" class="w-10 h-10 rounded-full object-cover border-2 border-edu-lime" />
          <div>
            <p class="text-sm font-bold">Sarah Chen</p>
            <p class="text-[11px] text-gray-400 dark:text-gray-500">UI/UX Designer at Google</p>
          </div>
        </div>
        <p class="text-sm text-gray-300 dark:text-gray-400 italic">"Curio transformed my career. The courses are incredibly well-structured."</p>
        <div class="flex gap-0.5 mt-2">
          <StarIcon v-for="i in 5" :key="i" class="w-3.5 h-3.5 text-edu-lime" />
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col justify-center items-center px-8 py-12 max-w-lg mx-auto w-full bg-edu-bg dark:bg-gray-900">
      ...
      <div class="w-full flex gap-3 mb-6">
        <button class="flex-1 flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-semibold text-edu-navy dark:text-edu-navy bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-soft">
          ...
          {{ i18n.t('auth.login.socialGoogle') }}
        </button>
        <button class="flex-1 flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-semibold text-edu-navy dark:text-edu-navy bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-soft">
          ...
          {{ i18n.t('auth.login.socialLinkedIn') }}
        </button>
      </div>
      ...
      <div class="w-full flex items-center gap-3 mb-6">
        ...
        <span class="text-xs font-medium text-edu-text-muted dark:text-gray-400">{{ i18n.t('auth.login.orEmail') }}</span>
        <div class="flex-1 h-px bg-gray-200 dark:bg-gray-700"></div>
      </div>
