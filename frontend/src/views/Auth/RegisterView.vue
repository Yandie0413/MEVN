...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMsg.value = e.response?.data?.message || (i18n.isFrench ? 'Impossible de créer le compte.' : 'Could not create account.');
  } finally {
...
<template>
  <div class="min-h-screen flex font-sans bg-edu-bg dark:bg-gray-900 relative">
    ...
    <div class="hidden lg:flex flex-col flex-1 bg-white dark:bg-gray-900 text-edu-navy dark:text-white p-12 justify-between relative overflow-hidden border-r border-gray-100 dark:border-transparent">
      <!-- Blobs -->
      <div class="absolute top-0 right-0 w-72 h-72 bg-edu-lime/20 dark:bg-edu-lime/10 rounded-full -translate-y-1/3 translate-x-1/3"></div>
      <div class="absolute bottom-20 left-0 w-48 h-48 bg-edu-lavender dark:bg-gray-800/30 rounded-full -translate-x-1/2"></div>
        ...
        <div class="flex flex-col gap-5">
          <div v-for="(step, i) in [i18n.t('auth.register.step1'), i18n.t('auth.register.step2'), i18n.t('auth.register.step3')]" :key="step"
            ...
            <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0"
                 :class="i === 0 ? 'bg-edu-lime text-edu-navy' : 'border border-gray-300 text-gray-400 dark:border-gray-600 dark:text-gray-500'">
              {{ i + 1 }}
            </div>
            <span class="text-sm" :class="i === 0 ? 'text-edu-navy dark:text-white font-semibold' : 'text-edu-text-muted dark:text-gray-500'">{{ step }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col justify-center items-center px-8 py-10 max-w-lg mx-auto w-full bg-edu-bg dark:bg-gray-900">
      ...
      <form @submit.prevent="handleSubmit" class="w-full flex flex-col gap-4">
          ...
          <div v-if="password" class="flex items-center gap-2 mt-1">
            <div class="flex gap-1 flex-1">
              <div v-for="i in 4" :key="i" class="h-1 flex-1 rounded-full transition-all duration-300"
                   :class="i <= passwordStrength() ? strengthColor : 'bg-gray-200 dark:bg-gray-700'"></div>
            </div>
        ...
        <!-- Role selector -->
        <div class="flex flex-col gap-1.5">
          <label class="text-sm font-semibold text-edu-navy dark:text-edu-navy">{{ i18n.isFrench ? 'Je m\'inscris en tant que' : 'I am signing up as' }}</label>
          <div class="grid grid-cols-2 gap-3">
            <button
              type="button"
              @click="role = 'etudiant'"
              class="flex items-center gap-2 justify-center py-3 rounded-xl text-sm font-semibold border transition-all"
              :class="role === 'etudiant' ? 'bg-edu-navy text-edu-lime border-edu-navy' : 'bg-white dark:bg-gray-800 text-edu-text-muted dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-edu-navy'"
            >
              <AcademicCapIcon class="w-4 h-4" /> {{ i18n.isFrench ? 'Étudiant' : 'Student' }}
            </button>
            <button
              type="button"
              @click="role = 'enseignant'"
              class="flex items-center gap-2 justify-center py-3 rounded-xl text-sm font-semibold border transition-all"
              :class="role === 'enseignant' ? 'bg-edu-navy text-edu-lime border-edu-navy' : 'bg-white dark:bg-gray-800 text-edu-text-muted dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-edu-navy'"
            >
              <BriefcaseIcon class="w-4 h-4" /> {{ i18n.isFrench ? 'Enseignant' : 'Teacher' }}
            </button>
          </div>
        </div>
