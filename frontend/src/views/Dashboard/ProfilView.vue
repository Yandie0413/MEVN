...
const user = {
  name: 'Léa Martin',
  role: i18n.isFrench ? 'Explorateur · Niveau 12' : 'Explorer · Level 12',
  streak: 7,
  xp: 2840,
  xpMax: 3200,
  level: 12,
...
const badges = computed(() => [
  { id: 1, label: i18n.t('dashboard.badgeFirstStep'), desc: i18n.isFrench ? 'Termine ton premier chapitre' : 'Complete your first chapter', icon: RocketLaunchIcon, bg: '#DCFCE7', color: '#16a34a', unlocked: true },
  { id: 2, label: i18n.t('dashboard.badgeMarathon'), desc: i18n.isFrench ? '7 jours d\'affilée' : '7 days in a row', icon: FireIcon, bg: '#FFEDD5', color: '#ea580c', unlocked: true },
  { id: 3, label: i18n.t('dashboard.badgeQuizMaster'), desc: i18n.isFrench ? '10 quiz réussis' : '10 quizzes passed', icon: LightBulbIcon, bg: '#E0E7FF', color: '#7c3aed', unlocked: true },
  { id: 4, label: i18n.t('dashboard.badgePolyglot'), desc: i18n.isFrench ? 'Termine un cours de langue' : 'Complete a language course', icon: GlobeAltIcon, bg: '#E0F2FE', color: '#0284c7', unlocked: true },
  { id: 5, label: i18n.t('dashboard.badgeScientist'), desc: i18n.isFrench ? 'Termine 3 cours de sciences' : 'Complete 3 science courses', icon: BeakerIcon, bg: '#F3F4F6', color: '#9ca3af', unlocked: false },
  { id: 6, label: i18n.t('dashboard.badgeLegend'), desc: i18n.isFrench ? 'Atteins le niveau 20' : 'Reach level 20', icon: TrophyIcon, bg: '#F3F4F6', color: '#9ca3af', unlocked: false },
]);
...
// Dynamic history: courses with progress > 0
...
const stats = computed(() => {
  ...
  const termines = courseStore.courses.filter(c => c.progress === 100).length;
  const certs = courseStore.courses.filter(c => c.progress === 100).length;

  return [
    { label: i18n.isFrench ? 'Cours suivis' : 'Courses followed', value: suivis || 5, icon: BookOpenIcon, bg: '#E0E7FF', color: '#7c3aed' },
    { label: i18n.isFrench ? 'Cours terminés' : 'Courses completed', value: termines, icon: CheckCircleIcon, bg: '#DCFCE7', color: '#16a34a' },
    { label: i18n.t('dashboard.certificatesCount'), value: certs || 3, icon: CheckBadgeIcon, bg: '#FEE2E2', color: '#e11d48' },
  ];
});
...
<template>
  <div class="flex flex-col">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[28px] shadow-soft dark:shadow-gray-900/50 mb-6 overflow-hidden">
      <div class="p-8" style="background: linear-gradient(135deg, #E0E7FF 0%, #E0F2FE 100%);">
        <div class="flex items-start justify-between">
          ...
          <div class="flex items-center gap-5">
            ...
            <div>
              <h1 class="text-2xl font-extrabold text-edu-text-main mb-1">{{ user.name }}</h1>
              <p class="text-edu-text-muted dark:text-gray-400 text-sm font-medium mb-4">{{ user.role }}</p>
              <!-- Mini stats -->
              <div class="flex items-center gap-4">
                <div class="flex items-center gap-1.5 text-xs font-bold text-edu-text-main">
                  <FireIcon class="w-4 h-4 text-orange-500" />
                  <span>{{ user.streak }} {{ i18n.t('dashboard.streak') }}</span>
                </div>
                <div class="flex items-center gap-1.5 text-xs font-bold text-edu-text-main">
                ...
                <div class="flex items-center gap-1.5 text-xs font-bold text-edu-text-main">
                  ...
                  <span>{{ i18n.t('dashboard.certificatesCount') }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Settings button -->
          <button class="bg-white/80 hover:bg-white text-edu-text-main text-sm font-bold px-5 py-2.5 rounded-2xl flex items-center gap-2 transition-colors shadow-sm backdrop-blur-sm">
            <Cog6ToothIcon class="w-4 h-4" />
            {{ i18n.isFrench ? 'Paramètres' : 'Settings' }}
          </button>
        </div>
        ...
        <div class="mt-6">
          <div class="flex justify-between text-xs font-bold text-edu-text-muted mb-2">
            <span>{{ i18n.isFrench ? 'Niveau' : 'Level' }} {{ user.level }}</span>
            <span>{{ user.xp }} / {{ user.xpMax }} XP</span>
            <span>{{ i18n.isFrench ? 'Niveau' : 'Level' }} {{ user.nextLevel }}</span>
          </div>
    ...
    <div class="flex gap-6 pb-10">
      ...
      <!-- Right Column -->
      <div class="w-[280px] flex-shrink-0 flex flex-col gap-6">
        ...
        <!-- Prochain palier -->
        <div class="bg-edu-navy dark:bg-gray-900 rounded-[24px] p-6 shadow-soft dark:shadow-gray-900/50 relative overflow-hidden">
          <!-- Decorative background icon -->
          <div class="absolute -right-4 -bottom-4 opacity-10">
            <TrophyIcon class="w-24 h-24 text-edu-lime" />
          </div>

          <p class="text-[10px] font-black tracking-[0.15em] text-edu-lime/70 uppercase mb-2">{{ i18n.isFrench ? 'Prochain palier' : 'Next level' }}</p>
          <h3 class="text-lg font-extrabold text-white mb-2">{{ i18n.isFrench ? 'Niveau' : 'Level' }} {{ user.nextLevel }} : {{ i18n.isFrench ? 'Aventurier' : 'Adventurer' }}</h3>
          <p class="text-xs text-white/70 leading-relaxed">
            {{ i18n.isFrench ? 'Plus que' : 'Only' }} <strong class="text-edu-lime">{{ user.xpMax - user.xp }} XP</strong> {{ i18n.isFrench ? 'pour débloquer un avatar exclusif !' : 'to unlock an exclusive avatar!' }}
          </p>
          <div class="mt-4 h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div
              class="h-full bg-edu-lime rounded-full"
              :style="{ width: xpPercent + '%' }"
            ></div>
          </div>
        </div>
