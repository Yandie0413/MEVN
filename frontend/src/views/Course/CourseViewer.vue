...
function getChapterStatus(chapter: any): string {
...
<template>
  <div class="min-h-screen font-sans pb-12" :class="viewMode === 'lesson' ? 'bg-edu-bg dark:bg-[#0B0B0F]' : 'bg-edu-bg dark:bg-gray-900'" v-if="currentCourse">

    <template v-if="viewMode === 'overview'">
      <!-- TOP BAR (Matches Image 2 top bar) -->
      <header class="max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div class="w-full sm:w-[450px] bg-white h-12 rounded-full flex items-center px-5 shadow-soft">
          <MagnifyingGlassIcon class="w-5 h-5 text-gray-400 mr-3 flex-shrink-0" />
          <input v-model="searchTerm" @keyup.enter="submitSearch" type="text" placeholder="Cherche un cours, un sujet..." class="bg-transparent border-none outline-none text-sm w-full text-edu-text-main placeholder-gray-400 font-medium" />
        </div>
        <div class="flex items-center gap-4">
          <div class="h-12 bg-white rounded-full flex items-center px-5 shadow-soft gap-2 font-bold text-sm text-edu-text-main flex-shrink-0">
            <FireIcon class="w-5 h-5 text-orange-500" />
            <span>{{ authStore.stats.streakDays }} <span class="text-gray-400 font-medium">jours</span></span>
          </div>
        </div>
      </header>
      ...
      <div class="max-w-7xl mx-auto px-6 mb-4 flex items-center">
        <button
          @click="router.push('/dashboard/mes-cours')"
    ...
    <!-- CINEMA MODE TOP BAR (immersive, sticky; dark by default, follows the site theme) -->
    <header v-else-if="viewMode === 'lesson'" class="sticky top-0 z-20 bg-white/85 dark:bg-[#0B0B0F]/85 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 mb-8">
      <div class="max-w-[1600px] mx-auto px-6 py-4 flex items-center gap-6">
        <button @click="goBackToOverview" class="flex items-center gap-2 text-xs font-bold text-edu-text-muted dark:text-white/60 hover:text-edu-lime transition-colors flex-shrink-0">
          <ArrowLeftIcon class="w-4 h-4" />
          <span class="hidden sm:inline">{{ i18n.t('course.backToOverview') }}</span>
        </button>
        <div class="flex-1 min-w-0">
          <div class="flex justify-between text-[10px] font-bold text-edu-text-muted dark:text-white/40 mb-1.5">
            <span class="truncate">{{ currentCourse.title }}</span>
            <span class="flex-shrink-0 ml-2">{{ progressPercentage }}%</span>
          </div>
          <div class="h-1.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
            <div
              class="h-full bg-gradient-to-r from-edu-lime to-emerald-400 rounded-full transition-all duration-700 shadow-[0_0_10px_rgba(212,240,99,0.5)]"
              :style="{ width: progressPercentage + '%' }"
            ></div>
          </div>
        </div>
        <span class="text-[11px] font-bold text-edu-text-muted dark:text-white/50 flex-shrink-0">{{ completedChaptersCount }} / {{ currentCourse.chapters.length }}</span>
      </div>
    </header>
          ...
          <div class="flex flex-col gap-4">
            ...
            >
              ...
              <div class="flex-shrink-0">
                ...
                <div v-else-if="getChapterStatus(chapter) === 'En cours'" class="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center border border-indigo-100">
              ...
              <div class="flex-shrink-0 text-right">
                <span 
                  ...
                  :class="[
                    ...
                    getChapterStatus(chapter) === 'En cours' ? 'bg-indigo-100 text-indigo-700' : '',
                    getChapterStatus(chapter) === 'À faire' ? 'bg-gray-100 text-gray-500' : ''
                  ]"
                >
    ...
    <!-- VIEW MODE: LESSON — Mode Cinéma (75% contenu / 25% navigation) -->
    <div class="max-w-[1600px] mx-auto px-6" v-else-if="viewMode === 'lesson'">
      <div class="flex flex-col lg:flex-row-reverse gap-8">

        <!-- Chapter navigation (25%, floating glass panel) -->
        <div class="w-full lg:w-[300px] flex-shrink-0">
          <div class="bg-black/[0.02] dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 rounded-3xl p-5 lg:sticky lg:top-28 backdrop-blur-sm">
            <p class="text-[10px] font-black text-edu-text-muted dark:text-white/40 uppercase tracking-widest mb-4 px-1">{{ i18n.t('course.chapters') }}</p>
            <div class="flex flex-col gap-1.5">
              <button
                v-for="(chapter, index) in currentCourse.chapters"
                :key="chapter.id"
                @click="courseStore.setActiveChapter(chapter.id)"
                class="w-full text-left p-3 rounded-2xl border text-xs font-bold transition-all duration-200 flex items-start gap-3 focus:outline-none"
                :class="[
                  activeChapter?.id === chapter.id
                    ? 'border-edu-lime/30 bg-edu-lime/10 text-edu-lime'
                    : 'border-transparent text-edu-text-muted dark:text-white/50 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-edu-text-main dark:hover:text-white/80'
                ]"
              >
                <span class="flex-shrink-0 mt-0.5">
                  <Transition name="pop" mode="out-in">
                    <CheckCircleIcon v-if="chapter.completed" key="done" class="w-4 h-4 text-emerald-400" />
                    <PlayIcon v-else-if="activeChapter?.id === chapter.id" key="active" class="w-4 h-4 text-edu-lime" />
                    <div v-else key="todo" class="w-4 h-4 rounded-full border border-gray-300 dark:border-white/20"></div>
                  </Transition>
                </span>
